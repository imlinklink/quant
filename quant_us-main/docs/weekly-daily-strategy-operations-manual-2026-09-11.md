...
用户决定不纳入退市样本。后续扩池和历史证据建设以 [存续普通股扩池与时点证据快照技术设计](historical-universe-and-evidence-handoff-technical-design-2026-09-12.md) 为准；本手册关于必须取得退市证券来源、纳入退市样本和实现退市结算的前置要求不再适用。历史回测只能报告所选存续普通股的条件性结果，不能据此推断历史全市场或声称消除了幸存者偏差。原有数据质量、时间戳和禁止伪造 LLM 标签要求继续有效。
...
> 2026-09-12 复核又修正了收盘价退出的仓位释放时间、`DATA_END` 右删失、Manifest 相对路径和父子组联合重采样。`BUY-WD-ABC-EXP-001` 保留作问题追溯，新试点必须使用新实验编号。
