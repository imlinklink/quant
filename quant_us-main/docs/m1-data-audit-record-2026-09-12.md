...
**判断（待验证的假设，非结论）**：`1526/11290` 说明影响面**不小**。这些 setup 的**特征尺度**被未来拆股改变；虽然本策略多数信号看似尺度不变（周线门用比值、止损/高开门用 close±k·ATR），且样本内拆股前价格均高于 $5 门槛，但**"方向性影响很小"目前只是推测**，必须由**修正口径后的配对重跑**（不复权执行价 + 逐日 as-of 特征）来检验，不得据此提前下结论。

按方案要求：**M2 必须**用**不复权执行价 + 逐决策日 as-of 特征**重建，不能用 QFQ 快照充当执行价或历史均线。
