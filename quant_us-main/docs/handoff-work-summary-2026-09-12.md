...
> 复核补记（2026-09-12）：下文记录的是交接时的提交和测试状态；后续复核又修复了证据包模式、标签完整性、ticker 归属、证据哈希及 as-of 价格快照问题，因此开头的 `HEAD`、工作区和通过数不代表当前状态。当前 39 只存续股票的 QFQ 回放仍是条件性诊断：Futu 上市日有占位/缺失，QFQ 重下载可能后验重算，不能据此宣称原始执行价与历史时点特征价已通过全样本验证；D 的严格证据层仍为 0。需在可核实的上市日、原始价/公司行动和前瞻 shadow 数据齐备后另建实验编号，不能改写已有实验产物。
