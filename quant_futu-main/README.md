...
默认是全自动下单。如需"信号先上页面、人工点『下单』才执行"：

```yaml
# config.yaml -> trading.live_trading.human_approval
enabled: true        # 打开人工确认
port: 8899           # 本地确认页端口
```

启动实盘后打开日志中打印的地址（默认 http://127.0.0.1:8899）。
买入信号会显示在页面上（含规则触发理由 + 大模型判定），
点「下单」才真正成交，点「拒绝」当天不再推送同一只。

注意：持仓风控（ATR 止盈止损、时间退出等）仍然是自动执行的，
人工确认只作用于"买入"环节，防止未经确认的自动买入。
