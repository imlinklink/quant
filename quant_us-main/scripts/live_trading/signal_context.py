    ...
    try:
        ...
        for _, row in data.head(count).iterrows():
            ...
            try:
                ...
                published_at = datetime.fromtimestamp(int(ts), ZoneInfo('UTC')).isoformat()
            except Exception:
                pass
            out.append({
                'title': row.get('title'),
                ...
                'time': time_txt,
                'url': row.get('url'),
                # 公告/评级 → filing（TTL 长），资讯 → news
                'kind': 'filing' if kind in ('NOTICE', 'RATING') else 'news',
...
def fetch_event_evidence(symbol: str, count: int = 5) -> List[Dict]:
    """拉取富途资讯/公告，转成不可变 evidence 列表（带 evidence_id/content_hash，可被 LLM 引用）。

    只调富途（中文源、稳定），不拉 Yahoo 财报 / FINRA 空头等不稳定外部接口。
