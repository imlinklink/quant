...
## D 组诊断（2026-09-12，真实富途财报，提交 `a838bf9`）

用富途 `get_financials_earnings_price_move` 取 39 只样本的财报**发布日期 + 盘前/盘后**，转成诊断层证据（`observed_at` 一律留空）：

- 证据 **1302 条**（39 只，2010-10 ~ 2026-09）；`published_at` 取保守口径（盘前/盘中 09:30 ET、盘后/未知 16:00 ET，夏令时正确）。
- 源审计通过；100/100 条缺 `observed_at`（符合预期，**不伪造首次可见时间**）。
- 取 500 个 setup 建 packet：

| 层 | 可见证据 | 结论 |
|---|---|---|
| **严格层** | **0**（全部 `OBSERVED_AT_UNPROVEN`/`SOURCE_UNVERIFIED`） | 富途财报**无法**支撑严格 D |
| 诊断层（放宽 observed_at 与来源核验） | 373/500 setup（**75%**）有 ≥1 条可见证据，中位 20 条/篇 | 证据量本身充足，但被"无 observed_at"与"模型后见知识"双重污染 |

**结论**：**D 组维持 `inconclusive`**。富途财报可用于诊断层与构建待标注清单，**不得**作为"LLM 有无增量"的证据；验证 LLM 真实增量只能靠样本冻结后的**前向 shadow**。
